...
const TotalWorkingTimeInMonths: FC = () => {
  const { t } = useTranslation();
  const { data, isPending, isError, refetch } = useQuery({
    queryKey: ["total-working-time-in-months"],
    queryFn: () => DayApi.totalWorkingTimeInMonths(),
  });
  return (
    <DashboardElement
      isError={isError}
      isLoading={isPending}
      refetch={refetch}
      title={t("totalWorkingTimeInMonths")}
    >
      <Bar
        data={{
...
export default TotalWorkingTimeInMonths;
